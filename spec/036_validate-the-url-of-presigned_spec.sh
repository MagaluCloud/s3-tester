Describe 'get-presign:' category:"BucketSharing" category:"quick"
  setup(){
  ...
  Example "on profile $1 using client $2" id:"036"
    ...
    presign_url=$(mgc object-storage objects presign --dst $test_bucket_name/$file1_name --server-url http://201.54.14.64)
...
Describe 'put-presign:' category:"BucketSharing"
  ...
  Example "on profile $1 using client $2" id:"036"
    ...
    "mgc")
    mgc workspace set $profile > /dev/null
    presign_url=$(mgc object-storage objects presign --dst $test_bucket_name/$file1_name --expires-in "5m" --method PUT --server-url http://201.54.14.64)
    presign_url=$(echo "$presign_url" | grep -oP 'https.*?host')
